@tool
class_name HalfCapsuleFlattened
extends Node3D

@export_range(0.0, 10.0) var height = 1.0 : 
    set(v): height = v; generate()
@export_range(0.0, 10.0) var radius = 0.5 : 
...
@export_range(0.0, 5.0) var roundRadius = 0.1 :
...
func generate():
    ...
    var cylt = (height-roundRadius)*Vector3.UP
    ...
    for seg in segments:
        ...
        for ring in range(rings):
            ...
            var w1 = Vector3.RIGHT * (radius - roundRadius) + Vector3.RIGHT.rotated(Vector3.FORWARD, a1) * roundRadius + cylt
            var w2 = Vector3.RIGHT * (radius - roundRadius) + Vector3.RIGHT.rotated(Vector3.FORWARD, a2) * roundRadius + cylt
